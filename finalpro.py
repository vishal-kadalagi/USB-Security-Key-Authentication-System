...
def generate_key():
        ...
        def send_otp(sender_email, otp):
            ...
            server.starttls()
            server.login('vinayak.rkamkar@gmail.com', 'kjcarjpdytrvbkvi')

            subject = 'Security Key'
            message = MIMEMultipart()
            message['From'] = 'vinayak.rkamkar@gmail.com'
            message['To'] = 'vishalkadalagi2004@gmail.com'
            message['Subject'] = subject

            body = 'Your OTP is ' + str(otp)
            message.attach(MIMEText(body, 'plain'))

            server.sendmail('vinayak.rkamkar@gmail.com', 'vishalkadalagi2004@gmail.com', message.as_string())
            print("OTP sent successfully")
            insert_db(str(otp))
            server.quit()

        security_key = ''.join(secrets.choice('0123456789') for _ in range(4))
        sender_email = 'vinayak.rkamkar@gmail.com'
...
##########-------Function to check if a removable disk is detected (Software)----------###########

def check_removable_disk():
    """
    Checks if a removable USB drive is detected and returns the drive letter if found.
    """
    try:
        out = subprocess.check_output('wmic logicaldisk get DriveType, Caption', shell=True)
        for drive in str(out).strip().split('\\r\\r\\n'):
            if '2' in drive:  # DriveType 2 indicates a removable drive
                drive_letter = drive.split()[0]  # Extract drive letter (e.g., H:)
                return drive_letter  # Return the detected drive letter
    except subprocess.CalledProcessError as e:
        print(f"Error checking removable disks: {e}")
        return None
...
def main():
    """
    Main function to check USB and navigate to the home page.
    """
    usb_drive = check_removable_disk()

    if usb_drive:
        print(f"USB Detected at {usb_drive}. Redirecting to home page...")
        home_page()  # Call the home page function directly
    else:
        messagebox.showerror("USB Not Detected", "Specified USB is not connected. The software will not run.")
        sys.exit()
...
##########-------Function to check if a removable disk is detected (Software) END----------###########
